...
<body style="text-align:center; background-color:#161616;">
    ...
    $ganadores = [
        "piedra" => ["tijeras" => "tijeras", "lagarto" => "lagarto"],
        "papel" => ["piedra" => "piedra", "spock" => "spock"],
        "tijeras" => ["papel" => "papel", "lagarto" => "lagarto"],
        "lagarto" => ["papel" => "papel", "spock" => "spock"],
        "spock" => ["piedra" => "piedra", "tijeras" => "tijeras"]
    ];
    $transformarNum = ["piedra", "papel", "tijeras", "lagarto", "spock"];

    if (isset($eleccion)) {
        
        $maquina = $transformarNum[rand(0, (count($transformarNum) - 1))];
        $contenido = "Algo ha pasado.";
        if ($eleccion == $maquina) {
            $contenido =  "EMPATE!!";
        } else if (isset($ganadores[$eleccion][$maquina])) {
