...
<div class="w-75" style="margin: 0 auto">
  ...
  <form action="index.php?accion=guardar&evento=crear&tabla=producto" method="POST">
    ...
    <div class="form-group">
      ...
      <input required type="text" class="form-control mb-4" id="nomproducto" name="nomproducto" placeholder="Introduce el Nombre de la pieza" value="<?= $_SESSION["datos"]["nompieza"] ?? "" ?>">
      <?= isset($errores["nompieza"]) ? '<div class="alert alert-danger" role="alert">' . DibujarErrores($errores, "nompieza") . '</div>' : ""; ?>
    </div>
    
    <div class="form-group">
      <label for="preciovent">Precio</label>
    <input required type="number" min="0" step="any" pattern="^[0-9]+[.,]?([0-9]{0,2})$" class="form-control mb-4" id="preciovent" name="preciovent" placeholder="Introduce el Precio" value="<?= $_SESSION["datos"]["preciovent"] ?? "" ?>">
    <?= isset($errores["preciovent"]) ? '<div class="alert alert-danger" role="alert">' . DibujarErrores($errores, "preciovent") . '</div>' : ""; ?>
  </div>

  <select required onchange="mostrarInputs(this)" class="form-select mb-4" aria-label="Default select example">
